...
tryCatch({
  ...
  rm(all_summary, dep_summary, ind_summary)
...
saveRDS(
  list(Table1 = table1, TableA1 = tableA1), 
...
tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "Overall_All_Applicants",
  calc_pct_change(df)
)

tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "Overall_Independent",
  calc_pct_change(df, "ind", 1)
)

tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "Overall_Dependent",
  calc_pct_change(df, "dep", 1)
)

# ZIP Median Household Income - Total Submissions
tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "MHI_First_Tercile",
  calc_pct_change(df, "mhi_1", 1)
)

tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "MHI_Second_Tercile",
  calc_pct_change(df, "mhi_2", 1)
)

tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "MHI_Third_Tercile",
  calc_pct_change(df, "mhi_3", 1)
)

# ZIP Non-White Share - Total Submissions
tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "NonWhite_First_Tercile",
  calc_pct_change(df, "nw_1", 1)
)

tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "NonWhite_Second_Tercile",
  calc_pct_change(df, "nw_2", 1)
)

tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "NonWhite_Third_Tercile",
  calc_pct_change(df, "nw_3", 1)
)

# ZIP SNAP Share - Total Submissions
tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "SNAP_First_Tercile",
  calc_pct_change(df, "snap_1", 1)
)

tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "SNAP_Second_Tercile",
  calc_pct_change(df, "snap_2", 1)
)

tableA2_rows[[length(tableA2_rows) + 1]] <- c(
  "SNAP_Third_Tercile",
  calc_pct_change(df, "snap_3", 1)
)
...
panel_a_app_rows <- data.frame(
  Row = c("Baseline Mean Refile Rate", 
          "", 
          "Refiled in 2024",
          "SE_2024",
          "",
...
# PREPARE DATA: Create age polynomials for logistic model
# Add age squared and age cubed to the regression dataset
...
# Clean up Table 3 objects (but keep logit models for Appendix Table A3)
...
# Clean up logit models and Table A3 objects
rm(logit_model_app, logit_model_cal, tableA3_output, coef_table)
gc()
